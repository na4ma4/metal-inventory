...
.PHONY: run
run: artifacts/build/debug/$(GOHOSTOS)/$(GOHOSTARCH)/mic
	"$<" $(RUN_ARGS)

.PHONY: run-admin
run-admin: artifacts/build/debug/$(GOHOSTOS)/$(GOHOSTARCH)/mica
	"$<" $(RUN_ARGS)

.PHONY: run-daemon
run-daemon: artifacts/build/debug/$(GOHOSTOS)/$(GOHOSTARCH)/micd
	"$<" $(RUN_ARGS)
